  ...
  try {
    ...
    if (!/^[a-f0-9]{32}$/i.test(key)) {
      res
        .status(500)
        .send(
          "TRELLO_API_KEY format looks invalid. Expected a 32-char hex key.",
        );
      return;
    }

    var fields = "id,name,desc,due,url,dateLastActivity";
    var endpoint =
    ...
    var etag =
      'W/"' + card.id + "-" + new Date(card.dateLastActivity).getTime() + '"';
