...
module.exports = async function handler(req, res) {
  try {
    ...
    if (!boardId) {
      res.status(400).send('Missing boardId. Pass ?boardId=... or set TRELLO_BOARD_ID env var.');
      return;
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        res.status(401).send('Trello auth failed (invalid key/token). Check TRELLO_API_KEY and TRELLO_API_TOKEN.');
        return;
      }
    ...
    var etag = 'W/"' + boardId + '-' + dueCards.length + '-' + maxActivity + '"';
